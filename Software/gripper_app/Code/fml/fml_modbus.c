...
unsigned char g_mb_address = 0x01; //从机地址
        ...
        if (rx_buf[1] == 5)
        {
          ...
          if (0 != mb_write_coil_cb(addr, val))
            error_code = 4;
					...
					{
						if(pWriteRegCb->wirte_coils_cb(rx_buf, rx_len) == RM_OK) //写线圈处理函数
						{
							if (0 != mb_write_coil_cb(addr, val))
								error_code = 4;
						}
...
{
	g_mb_address = SYSTEM_PARAM()->device_id;
	pWriteRegCb = _cb;
	
	//寄存器初始化
	fml_modbus_update_reg(E_REG_HOLDING, MB_REG_DEV_ID, g_mb_address); //设备ID
...
void fml_modbus_reg_update(void)
{
	g_dio_state[0].pin_state = DIN1;
	g_dio_state[1].pin_state = DIN2;
	
	fml_modbus_update_reg(E_REG_COILS, MB_COILS_DIO_MODE_0, g_dio_state[0].pin_mode);
	fml_modbus_update_reg(E_REG_COILS, MB_COILS_DIO_STATE_0, g_dio_state[0].pin_state); 
	fml_modbus_update_reg(E_REG_COILS, MB_COILS_DIO_MODE_1, g_dio_state[1].pin_mode); 
	fml_modbus_update_reg(E_REG_COILS, MB_COILS_DIO_STATE_1, g_dio_state[1].pin_state);
}